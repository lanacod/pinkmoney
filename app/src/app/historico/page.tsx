  ...
  return (
    <div className="px-5 pt-6 pb-4 space-y-4">
      ...
      <div className="flex items-center justify-between">
        ...
        <Link href="/nova-transacao" className="w-9 h-9 rounded-full pm-btn-primary flex items-center justify-center">
          <Plus size={18} />
        </Link>
      </div>

      {/* Busca */}
      <div className="flex items-center gap-2 px-4 py-3 rounded-xl pm-glass border border-[var(--pm-outline-variant)]">
        <Search size={16} className="text-[var(--pm-outline)] flex-shrink-0" />
        <input
          type="text"
          value={search}
      ...
      <div className="flex gap-2">
        {([['all', 'Todos'], ['income', 'Entradas'], ['expense', 'Saídas']] as const).map(([val, label]) => (
          <button
            key={val}
      ...
      {/* Resumo */}
      <GlassCard className="flex divide-x divide-[var(--pm-outline-variant)]">
        <div className="flex-1 flex items-center gap-2 p-4">
          <TrendingUp size={16} className="text-emerald-400" />
          <div>
            <p className="text-[10px] text-[var(--pm-on-surface-variant)] uppercase tracking-wide">Entradas</p>
            <p className="pm-numeric text-sm font-bold text-emerald-400">{formatBRL(income)}</p>
          </div>
        </div>
        <div className="flex-1 flex items-center gap-2 p-4">
          <TrendingDown size={16} className="text-[var(--pm-primary-container)]" />
          <div>
            <p className="text-[10px] text-[var(--pm-on-surface-variant)] uppercase tracking-wide">Saídas</p>
      ...
      {isLoading ? (
        <div className="space-y-3">
          {[...Array(5)].map((_, i) => (
            <div key={i} className="h-16 rounded-xl bg-[var(--pm-surface-container-high)] animate-pulse" />
          ))}
        </div>
      ) : filtered.length === 0 ? (
        <div className="text-center py-16 space-y-3">
          <p className="text-4xl">✨</p>
          <p className="text-[var(--pm-on-surface-variant)]">Nenhuma transação encontrada</p>
        ...
        <div className="space-y-5">
          {groups.map(([dateLabel, items]) => (
            <div key={dateLabel}>
              <p className="text-xs font-bold text-[var(--pm-on-surface-variant)] uppercase tracking-widest mb-2 px-1">
                {dateLabel}
              </p>
                    ...
                    <div key={t.id} className="flex items-center gap-3 p-4">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-2 mt-0.5">
                          <span className="text-[10px] text-[var(--pm-on-surface-variant)]">{formatDate(t.date)}</span>
                          {t.categories && (
                            <span className="text-[10px] px-1.5 py-0.5 rounded-full bg-[var(--pm-primary-container)]/20 text-[var(--pm-primary)]">
